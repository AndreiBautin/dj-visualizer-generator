...
Remaining for Andrei:

1. Make the repository public (Settings → General → Danger Zone, or the visibility toggle). Render can already serve a private repo, but a portfolio needs the source visible.
2. Optionally set topics on the repo page (dotnet, react, ffmpeg, clean-architecture, video).
3. Optionally protect the default branch once public.

No code changes required for the showcase pass. This commit only records the status so the portfolio queue can move on.
